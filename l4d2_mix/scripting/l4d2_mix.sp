...
public Action Command_Captainvote(int client, int args)
{
	...
	{
		CPrintToChat(client,"{default}[{olive}Mix{default}] Can't start a mix, why do you need 1v1 mix ?");
		return Plugin_Handled;
	}
	...
	if (0 < args)
	{
		...
		if (g_CvarMixStatus.IntValue && g_bTeamRequested[2] && g_bTeamRequested[3])
		{
			...
			{
				CPrintToChat(client, "{default}[{olive}Mix{default}] Spectators cannot cancel a mix. You Dumb Fuck!");
			}
		}
		else
		{
	...
	{
		CPrintToChat(client, "{default}[{olive}Mix{default}] Spectators cannot start a mix. You Dumb Fuck!");
		return Plugin_Handled;
	}
	...
	{
		CPrintToChat(client, "{default}[{olive}Mix{default}] Your team already requested a mix. You Dumb Fuck!");
		return Plugin_Handled;
	}
...
public Action TimerCheckSurvivorCaptainVote(Handle timer)
{
	if (g_CvarMixStatus.IntValue)
	{
		...
		else
		{
			...
			{
				CPrintToChatAll("{default}[{olive}Mix{default}] Surprise! First Chosen Captain left the server, mix process has been stopped.");
				g_CvarMixStatus.SetInt(5, false, false);
				return Plugin_Handled;
			}
...
void DisplayVoteMenuPlayerSelect()
{
	if (g_CvarMixStatus.IntValue)
	{
		...
		if (!IsValidClient(g_iSurvivorCaptain))
		{
			CPrintToChatAll("{default}[{olive}Mix{default}] Surprise! First Captain left the server, mix process has been stopped.");
			g_CvarMixStatus.SetInt(5, false, false);
			return;
		}
		if (!IsValidClient(g_iInfectedCaptain))
		{
			CPrintToChatAll("{default}[{olive}Mix{default}] Surprise! Second Captain left the server, mix process has been stopped.");
			g_CvarMixStatus.SetInt(5, false, false);
			return;
		}
...
public Action TimerCheckInfectedCaptainVote(Handle timer)
{
	if (g_CvarMixStatus.IntValue)
	{
		...
		else
		{
			...
			if (!IsValidClient(g_iInfectedCaptain))
			{
				CPrintToChatAll("{default}[{olive}Mix{default}] Surprise! Second Chosen Captain left the server, mix process has been stopped.");
				g_CvarMixStatus.SetInt(5, false, false);
				return Plugin_Handled;
			}
